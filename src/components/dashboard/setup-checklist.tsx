  ...
  return (
    <div className="rounded-2xl border border-sky-500/20 bg-gradient-to-br from-sky-500/5 to-transparent p-5">
      ...
      <div className="space-y-1">
        {steps.map((step, i) => (
          <Link key={i} href={step.href} target={step.external ? "_blank" : undefined}
            className={`flex items-center gap-3 p-3 rounded-xl transition-colors group ${step.done ? "opacity-40 cursor-default pointer-events-none" : "hover:bg-white/5 cursor-pointer"}`}
          >
            {step.done
