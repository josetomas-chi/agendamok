                  ...
                  {getBookingsForCourt(court.id).map(b => {
                    ...
                    return (
                      <div key={b.id}
                        ...
                        style={{ top, height,
                          background: (b.status === "COMPLETED" || Number(b.paidAmount) >= Number(b.price)) ? "rgba(34,197,94,0.55)" : b.coach?.color ? `${b.coach.color}cc` : "rgba(201,168,76,0.85)",
                          borderLeft: `3px solid ${(b.status === "COMPLETED" || Number(b.paidAmount) >= Number(b.price)) ? "#16a34a" : b.coach?.color ?? "#C9A84C"}`,
