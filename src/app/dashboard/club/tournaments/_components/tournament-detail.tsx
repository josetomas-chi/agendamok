...
import { ArrowLeft, UserPlus, Play, Trophy, X, Check, ChevronRight, Swords, Tag, Plus, Pencil, Lock, Unlock, Link2, ImagePlus, Loader2, GripVertical, RefreshCw, CalendarDays, List, Bell } from "lucide-react"
...
type Participant = { id: string; name: string; email: string | null; phone: string | null; players: { name: string; email?: string; phone?: string }[]; seed: number | null; status: string; group: string | null; categoryId: string | null; restrictions?: { date: string; time: string }[] }
type Match = {
      ...
      {tab === "participants" && (
        <div className="space-y-3">
          ...
          {participants.length === 0 ? (
            ...
            <>
            {(() => {
              const phones = participants.flatMap(p => {
                const nums: string[] = []
                if (p.phone) nums.push(p.phone.replace(/\D/g, ""))
                if (Array.isArray(p.players)) {
                  (p.players as { phone?: string }[]).forEach(pl => {
                    if (pl.phone) nums.push(pl.phone.replace(/\D/g, ""))
                  })
                }
                return nums
              }).filter(Boolean)
                ...
                <div className="flex flex-wrap gap-2">
                  <button
                    ...
                    onClick={() => {
                      ...
                      toast.success(`${phones.length} números copiados al portapapeles`)
                    }}
                    ...
                    Copiar números para grupo WhatsApp ({phones.length})
                  </button>
                  <button
                    type="button"
                    onClick={() => setNotifyOpen(true)}
                    className="flex items-center gap-2 px-3 py-2 rounded-xl text-xs font-bold uppercase tracking-wide transition-all"
                    style={{ background: "rgba(56,189,248,0.1)", border: "1px solid rgba(56,189,248,0.25)", color: "#0ea5e9" }}>
                    <Bell className="w-3.5 h-3.5" />
                    Notificar por email ({participants.filter(p => p.email).length})
                  </button>
                </div>
              )
            })()}
...
function MatchRow({ match: m, canEdit, onEdit }: { match: Match; canEdit: boolean; onEdit: () => void }) {
  ...
  return (
    <div style={{ borderBottom: "1px solid rgba(201,168,76,0.08)" }}>
      ...
      {notifyOpen && tournament && (() => {
        const recipients = allParticipants
          .filter(p => p.status === "REGISTERED")
          .flatMap(p => {
            const list: { name: string; email: string }[] = []
            if (p.email) list.push({ name: p.name, email: p.email })
            if (Array.isArray(p.players)) {
              for (const pl of p.players as { name: string; email?: string }[]) {
                if (pl.email && pl.email !== p.email) list.push({ name: pl.name, email: pl.email })
              }
            }
            return list
          })
        return (
          <NotifyModal
            ...
            contextLabel={`${recipients.length} inscritos — ${tournament.name}`}
